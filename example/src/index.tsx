...
export default function App(): React.JSX.Element {
  ...
  const handleOrientationUpdate = useCallback(
    (event: OrientationEvent) => {
      ...
      // Mark coverage when a new frame was actually captured
            ...
            onPress: async () => {
              ...
              try {
                const shots = frames.map((f: any, i: number) => {
                  console.log(
                    `[FRAME] idx=${i} row=${f.gridRow ?? '?'} col=${f.gridCol ?? '?'} ` +
                    `target=(${(f.targetYaw ?? 0).toFixed(1)}, ${(f.targetPitch ?? 0).toFixed(1)}) ` +
                    `measured=(${f.yaw.toFixed(1)}, ${f.pitch.toFixed(1)})`,
                  );
                  return {
                    path: f.path,
                    yaw: f.yaw,
  ...
  // Preview mode — show stitched equirectangular panorama
  ...
  // Capture mode — camera + grid overlay
